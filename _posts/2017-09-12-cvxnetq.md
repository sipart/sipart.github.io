...
[NetQ](https://cumulusnetworks.com/products/netq/) is a telemetry-based fabric validation system. You can try this out for free using Cumulus VX with the important caveat that you can't try out the NETQ [telemetry server](https://docs.cumulusnetworks.com/display/NETQ/Getting+Started+with+NetQ) unless you have a Cumulus licence to download the VM. But you can setup all the other components and run NETQ validation commands that query the REDIS server to see the fabric state.
