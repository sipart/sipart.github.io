...
>The SDN brouhaha forced vendors to give an appearance of becoming “software defined”, so most of them came up with something resembling a REST API (there were notable exceptions like Junos that had a good API from day one). Engineers who figured out that SDN means Still Does Nothing started thinking about network automation as SDN Lite thingy that could actually make their lives better. A lot of us started evangelizing the need for automation, which might have shifted the mindset a bit. Cloud happened for real – and once an organization starts deploying their workload in the cloud, you can either get your together and deliver services in reasonable time, or become obsolete. 
