...
By completing the below you are in effect creating a Ansible control node. Ansible then communcates with devices over SSH (no agent is needed on the managed devices). Below we are also setting netconf on Juniper devices to leverage the native device API.
...
As mentioned at the top SSH is used to communicate with a device so a user account on the device called ansilble is needed with the SSH key and for Juniper devices we will utilise the netconf API to push and pull from the Juniper devices so that needs configuring; Juniper example config below:
